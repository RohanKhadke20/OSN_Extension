...
|---|---|---|---|
| Manifest version | **3** | **3** (Event Page model) | **3** |
...
cleared when the service worker terminates. Firefox Manifest V3 (using the `background.scripts` Event Page model) and some Safari builds
do not expose it.
...
| `storage.managed_schema` | Retained (preserved in manifest for cross-browser policy compatibility) |
