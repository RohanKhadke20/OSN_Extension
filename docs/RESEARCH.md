...
| **CWE-1333** (Regular Expression Denial of Service - ReDoS) | Catastrophic backtracking in user-supplied or built-in regular expressions freezes the UI thread or extension background worker when evaluating adversarial input strings. | Medium (CVSS 5.3) | `src/core/pii-analyzer.js`, `src/content/scanner.js` | 1. `isSafeRegexPattern()` static analyzer rejects nested quantifiers (e.g., `(a+)+`, `(a|a)+`, `([a-z]+)*`), excessive pattern length (> 150 chars), and unbounded backreferences before compiling custom PII regexes.<br>2. Input text length clamped (`text.slice(0, 50000)` in scanner, `5000` in context menus).<br>3. LRU-capped regex compilation cache (maximum 100 compiled patterns). |
